...
sim_matrix = foreach(i = 1:num_shards, .combine = rbind, .packages = c("dplyr","reshape2")) %dopar% {
  ...
  for(j in 1:nrow(D_subset)){
    ...
    for(k in 1:length(K)){
      ...
      pred_pc_cs = prediction_evaluation_function(train_set = D_train, test_set = D_test_i, similarity_vector = B_cs)
      # pred_pc_pwc is one obs from test set
    ...
    if(nrow(B) > 1){
      ...
      for(k in 1:length(K)){
        ...
        pred_lirau = prediction_evaluation_function(train_set = D_train, test_set = D_test_i, similarity_vector = B_lirau)
        # pred_pc_pwc is one obs from test set
    ...
    if(nrow(B) > 1){
      ...
      for(k in 1:length(K)){
        ...
        pred_lira_multi = prediction_evaluation_function(train_set = D_train, test_set = D_test_i, similarity_vector = B_lira_multi)
        # pred_pc_pwc is one obs from test set
    ...
    if(nrow(B) > 1){
      ...
      for(k in 1:length(K)){
        ...
        pred_lira_lrt = prediction_evaluation_function(train_set = D_train, test_set = D_test_i, similarity_vector = B_lira_lrt)
        # pred_pc_pwc is one obs from test set
    ...
    if(nrow(B) > 1){
      ...
      for(k in 1:length(K)){
        ...
        pred_lira_gauss = prediction_evaluation_function(train_set = D_train, test_set = D_test_i, similarity_vector = B_lira_gauss)
        # pred_pc_pwc is one obs from test set
    ...
    if(nrow(B) > 1){
      ...
      for(k in 1:length(K)){
        ...
        pred_lira_bin = prediction_evaluation_function(train_set = D_train_temp, test_set = D_test_i, similarity_vector = B_lirau_bin)
        # pred_pc_pwc is one obs from test set
